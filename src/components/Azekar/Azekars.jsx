...
export default function Azekar() {
    ...
    const showData = dataAzekar.map((item, key) => (
        <Link
            ...
            {item.category}
            <div className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-orange-500 to-orange-400 transform -translate-x-full group-hover:translate-x-0 transition-transform duration-500"></div>
        </Link>
    ));
